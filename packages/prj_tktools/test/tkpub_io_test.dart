...
void main() {
  ...
  var configExportPath = normalize(
    absolute(
      join(
        '.dart_tool',
  ...
  test('tkpub config get/set package', () async {
    ...
    await shell.run(
      'tkpub config set demo --git-url url@git --git-path gitpath',
    );
  ...
  test('tkpub add', () async {
    ...
    expect(
      pubspecYamlHasAnyDependencies(pubspecYaml, ['tekartik_firebase']),
      isFalse,
    );
